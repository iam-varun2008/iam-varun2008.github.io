...
/**
 * HeroText Component - admissions-facing identity reveal
 */
const HeroText = ({ position = [0, 0.3, 0] }) => {
    ...
    // Letter positions for the VARUN split effect
    const letters = useMemo(() => {
        const chars = PROFILE.brand.split('');
        const center = (chars.length - 1) / 2;
        return chars.map((char, index) => ({
            char,
            baseX: (index - center) * 0.58,
            splitDir: index - center,
        }));
    }, []);

    // Tagline words for split effect
    const taglineWords = useMemo(() => {
        const positions = [-1.45, -0.5, 0.5, 1.45];
        const splitDirections = [-2, -0.7, 0.7, 2];
        return PROFILE.hero.categories.map((text, index) => ({
            text,
            baseX: positions[index],
            splitDir: splitDirections[index],
        }));
    }, []);
    ...
    useFrame((state, delta) => {
        ...
        // Apply split to each letter of VARUN
