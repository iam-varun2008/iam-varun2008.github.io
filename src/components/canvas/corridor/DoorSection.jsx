...
const DOOR_TEXTURES = {
    ...
    'PROJECTS': '/textures/corridor/doors/drzwiprojekty.webp',
    'THE STUDIO': '/textures/corridor/doors/drzwiarts.png',
    'ARTS': '/textures/corridor/doors/drzwiarts.png',
...
const DOOR_PAINTED_TEXTURES = {
    ...
    'PROJECTS': '/textures/corridor/doors/drzwiprojekty_painted.webp',
    'THE STUDIO': '/textures/corridor/doors/drzwiarts.png',
    'ARTS': '/textures/corridor/doors/drzwiarts.png',
...
const DoorSection = ({
        ...
        : roomId === 'gallery'
            ...
            : roomId === 'studio'
                ...
                : roomId === 'contact'
                    ? 'CONTACT'
                    : label;
    const doorTexturePath = DOOR_TEXTURES[textureKey] || DOOR_TEXTURES['THE GALLERY'];
    ...
    const doorPaintedTexturePath = DOOR_PAINTED_TEXTURES[textureKey] || DOOR_PAINTED_TEXTURES['THE GALLERY'];
    ...
    const wallSpan = isEndDoor ? WALL_X_OUTER * 2 : WALL_LENGTH;
    const doorBoardWidth = (wallSpan - 1.1) / 2;
    ...
    }, [holeWidth, holeHeight, holeOffsetY, wallSpan]);

    // Tilt parameters
    const BASE_ROTATION = isEndDoor ? 0 : Math.PI / 2;
    const BASE_TILT = isEndDoor ? 0 : 0.02;
    const MAX_TILT = isEndDoor ? 0 : BASE_WALL_ANGLE + 0.1;
    ...
    const pivotX = isEndDoor ? position[0] : (side === 'left' ? -WALL_X_OUTER : WALL_X_OUTER);

    // Wall offset from pivot - wall extends FROM pivot INWARD
    const wallOffsetX = isEndDoor
        ? 0
        : side === 'left'
            ? WALL_LENGTH / 2
            : -WALL_LENGTH / 2;
    ...
    useFrame(() => {
        ...
        const currentRotation = isEndDoor
            ? 0
    ...
    const handleClick = useCallback((e) => {
        ...
        if (e && e.isTeleport) {
            ...
            const corridorGlanceY = isEndDoor ? 0 : (side === 'left' ? 0.15 : -0.15);
        ...
        const cameraTargetZ = isEndDoor ? doorWorldPos.z + 2.2 : doorWorldPos.z;
        const cameraTargetX = isEndDoor ? 0 : (side === 'left' ? DOOR_ALIGN_X : -DOOR_ALIGN_X);
        ...
        const worldTargetRotationY = isEndDoor
            ? 0
            : side === 'left'
                ? DOOR_LOOK_ANGLE
                : -DOOR_LOOK_ANGLE;
    ...
    }, [camera, side, isOpen, isAnimating, setCameraOverride, isFastTeleport, isEndDoor, position]);
            ...
            onComplete: () => {
                ...
                gsap.to(camera.position, {
                    ...
                    onComplete: () => {
                        ...
                        // Commit the room immediately. The old 250ms delay left the
                        // camera inside an unpainted vestibule and caused a visible flash.
