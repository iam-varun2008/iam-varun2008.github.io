...
const CAMERA_SETTINGS = {
    ...
    forceCenterY: 0, // Contact now sits in the centered final doorway
...
const ContactRoom = ({ showRoom, onReady, isExiting, isWarmup }) => {
    ...
    const { hidePopup } = useAchievements();
    ...
    // ===== PAINT TRANSITION =====
    // Contact now opens from the centered final doorway.
    ...
    return (
        <group ref={groupRef} position={[0, -0.7, 0]}>
            ...
            <SocialBarrel
                position={size.width < 768 ? [-1.05, 0.1, -9.5] : [-2.3, 0.1, -9.5]}
                rotation={[0, 0.12, 0]}
                scale={size.width < 768 ? [1.4, 1.52] : [1.7, 1.84]}
