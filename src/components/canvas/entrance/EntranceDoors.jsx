...
const EntranceDoors = ({
    ...
    const disablePaintedTextures = typeof window === 'undefined'
    ...
    const doorRightPaintedTexture = useTexture(disablePaintedTextures ? dummyTex : '/textures/doors/door_right_painted.webp');
    const doorLeftPaintedTexture = useTexture(disablePaintedTextures ? dummyTex : '/textures/doors/door_left_painted.webp');
    const handleLeftTexture = useTexture('/textures/doors/handle_left_sketch.webp');
    const handleLeftPaintedTexture = useTexture(disablePaintedTextures ? dummyTex : '/textures/doors/handle_left_painted.webp');
    const handleRightTexture = useTexture('/textures/doors/handle_right_sketch.webp');
    const handleRightPaintedTexture = useTexture(disablePaintedTextures ? dummyTex : '/textures/doors/handle_right_painted.webp');

    // Dynamic textures for mobile
    const doorBackTexture = useTexture(disablePaintedTextures ? '/textures/doors/door_back.webp' : '/textures/doors/door_back_left_sketch.webp');
    const edgeTexture = useTexture(disablePaintedTextures ? '/textures/doors/pien_sketch.webp' : '/textures/doors/pien.webp');
    ...
    const handleClick = (e) => {
        ...
        // Camera flies through and stops near the profile display.
        ...
        // Keep the landing view centered and level while passing through the
        // connector, matching the intended first corridor composition.
    ...
    const handlePointerEnter = () => {
        if (isOpen || isAnimating || !canHover) return;
        setIsHovered(true);
    ...
    return (
        <group ref={groupRef} position={[position[0], 0, position[2]]}>
            ...
            */}
            ...
            {[
                { text: "CYBERSECURITY", x: -1.8 },
                { text: "PYTHON", x: -0.6 },
                { text: "TELANGANA", x: 0.6 },
                { text: "GRADE 12 · 97.9%", x: 1.8 }
            ].map((banner, index) => (
            ...
            {/* One continuous hit area keeps the seam, handles and artwork fully interactive. */}
            <mesh
                position={[0, doorCenterY, 0.115]}
                onClick={handleClick}
                onPointerEnter={handlePointerEnter}
                onPointerLeave={handlePointerLeave}
            >
                <planeGeometry args={[doorOpeningWidth, doorHeight]} />
            ...
            {/* <pointLight
                ...
                distance={10}
            /> */}
            {/* Personal monogram - used because no profile photograph was supplied */}
            <group
                ref={windowAvatarRef}
                position={[3.5, 0, 0.04]}
                rotation={[0, 0, 0]}
            >
                <mesh>
                    ...
                    <meshBasicMaterial color="#fcf3c6" transparent opacity={0.92} />
                </mesh>
                <Text
